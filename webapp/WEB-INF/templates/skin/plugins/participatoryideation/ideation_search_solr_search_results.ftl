...
<#if facets_list??>
	<#list facets_list as facet>
		...
		<#elseif facet?starts_with("handicap_string")>					<#assign facet_handicap = facet?split(":")?last>
...
<#--
	...
	facet_handicap = <strong>${facet_handicap!'?'}</strong><br>
			...
			<div id="search-prj-fields" class="container form">
				...
				<div id="searchParameters" class="collapse dont-collapse-sm">
					...
					<div class="row">
						...
						<div class="col-md-3 col-xs-12">
							<label class="select" for="handicap">
								<select name="fq" id="handicap" onChange="searchArrond()" class="form-control <#if facet_handicap != "">selected</#if>">
									<#if facet_handicap="">
										<option value="" selected="selected">#i18n{participatoryideation.search.form.field.handicap.label}</option>
									<#else>
										<option value="">#i18n{participatoryideation.search.form.field.handicap.reset}</option>
									</#if>
									...
									<#if facets??>
										<#list facets as facet>
											<#if facet.name == "handicap_string">
												<#list facet.values?sort as value>
													<#if value.count != 0>
														<#if facet_handicap == value.name>
															<option value="handicap_string:${(value.name)!''}" selected="selected" >${(value.name)!''} (${(value.count)!''})</option>
														<#else>
															<option value="handicap_string:${(value.name)!''}" >${(value.name)!''} (${(value.count)!''})</option>
														</#if>
													</#if>
												</#list>
											</#if>
		...
		<div id="sub-search" class="container">
			<div class="row">
				...
				<div id="sub-search-left-panel" class="col-xs-6 col-sm-6 col-md-6">
					<#if conf_user_query = "list_proposals">
...
<script type="text/javascript">
	...
	<#if conf_user_query = "list_proposals">
		<#assign other_view_code='map_proposals'>
	<#else>
		<#assign other_view_code='list_proposals'>
	</#if>
	
	$("#button_map_list").click( function(){
		$("#form_conf_hidden").attr('value',"map_proposals");
	});
	
	$("#button_list").click( function(){
		$("#form_conf_hidden").attr('value',"list_proposals");
	});
