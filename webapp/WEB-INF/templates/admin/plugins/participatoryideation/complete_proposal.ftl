...
<@rowBoxHeader i18nTitleKey="participatoryideation.complete_proposal.pageTitle">
	...
	<form class="form-horizontal" method="post" name="create_proposal" action="jsp/admin/plugins/participatoryideation/ManageProposals.jsp">
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-3" for="handicap">#i18n{participatoryideation.complete_proposal.labelHandicap} : </label>
			<div class="col-xs-12 col-sm-9 col-md-9">
				<select id="handicap" name="handicap" class="form-control">
					<#list handicap_list as handicap >
						<option value="${handicap.code}" <#if proposal.handicap?? && handicap.code==proposal.handicap> selected="selected"</#if>>${handicap.name}</option>
					</#list>
				</select>
				<p class="help-block">#i18n{participatoryideation.complete_proposal.labelHandicap.help}</p>
			</div>
		</div>
